...
(c) Select two or three further covariates from the data and fit a logistic regression with these additional covariates. Comment on your result.
...
Alternatively, we can use the `predict()` function in `R` as follows. The clever way to use `predict()` is to create a new simple data.frame containing data for three new students and then asking it to predict their responses. Or you can use predict three times.
